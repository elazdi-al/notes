...
\begin{minipage}[t]{0.48\textwidth}
	...
	\fbox{\begin{minipage}[htp]{1\textwidth}
			...
			\begin{minipage}[t]{0.51\textwidth}
				\textbf{Default.} Choice assigned in absence of active decision, without removing choice\\
				$\rightarrow$ auto subscription renewal, newsletters $\rightarrow$ significant effect on chosen option.\\
				- \textbf{Opt-in.} Checkbox unticked $\rightarrow$ user actively ticks.\\
				- \textbf{Opt-out.} Checkbox ticked $\rightarrow$ user actively unticks $\rightarrow$ doubled organ donors \textit{(40\% $\rightarrow$ 80\%)}.\\
			...
			\begin{minipage}[t]{0.47\textwidth}
				...
				$\rightarrow$ ChatGPT produces plausible text, no warranty it is correct.\\
				$\rightarrow$ Called ``hallucinations''/errors; in artificial neural networks; can cause harm but also be valuable; error rate hard to know.
			\end{minipage}\\
