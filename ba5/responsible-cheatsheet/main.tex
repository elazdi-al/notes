...
\fbox{
	\begin{minipage}[htp]{0.5\textwidth}
		...
		\textbf{Ethical Decision Making.}\\ \textbf{Recognize}(knowledge+experience+habits)$\to$\textbf{Evaluate}(cognitive+reason+affective)$\to$\textbf{Act}.\\
		\textbf{Ethical Lenses.}\\
		1. \textbf{Desirability.} The product corresponds to what people want.\\
		2. \textbf{Viability.} The product generates profits for a business.\\
		3. \textbf{Feasibility.} The product can be created with existing or new technology.\\
		\textbf{Fourth criterion (Isaac et al.).} Add \textbf{Ethicality}: the product should also be ethical.
		\hline
		...
		\end{minipage}\hfill
		\begin{minipage}[htp]{0.64\textwidth}
			\textbf{Cognitive side (Normative Ethics).}\\
			- \textbf{Utilitarianism} Consequences: choose the greatest good for greatest number\\
			- \textbf{Deontology} Action: do the right thing according to universal principles\\
			- \textbf{Virtue} Person: act as a virtuous person\\
			- \textbf{Care} Relationships: mutual responsibility and care for each other
		\end{minipage}\\
		...
		\textbf{Ethical Decision Making Frameworks.}\\
		...
		\hline
		\textbf{Stakeholder} Individuals, groups, organizations, institutions, and societies that might reasonably be affected by technology.\\
		\textbf{Direct Stakeholder.} \textit{in contact} with the product \textit{(End-users, Admins, Contractors, Developers)}.\\
		\textbf{Indirect Stakeholder.} affected through \textit{consequences of the usage} of the software \textit{(Environment, Government, Public(s), NGOs, Friends, Neighborhood)}.\\
		...
		Create a fictional story in two parts.\\
		1. Check pessimistic outcomes \textit{Black Mirror}.\\
