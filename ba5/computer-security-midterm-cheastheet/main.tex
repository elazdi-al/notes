...
\begin{minipage}[htp]{0.49\textwidth}
	...
	\textbf{UNIX Model.}\\
	Everything is a file. Each file has:
	\begin{itemize}[leftmargin=1.2em]
		\item[-] Owner (UID) and Group (GID)
		\item[-] 9 permission bits: 3 actions × 3 subjects
		      \begin{itemize}[leftmargin=1.2em]
			      \item \textit{Actions:} Read (r), Write (w), Execute (x)
			      \item \textit{Subjects:} Owner, Group, Others
		      \end{itemize}
	\end{itemize}
	\textit{Directories:} Read → list files; Write → add/remove files; Exec → traverse (cd).\\
