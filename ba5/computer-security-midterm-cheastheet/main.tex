...
\noindent \begin{minipage}[htp]{0.49\textwidth}
	...
	\begin{itemize}
		...
		\item[2.]\textbf{Fail-safe defaults.} Base access decisions on permission rather than exclusion. \textit{(Whitelist, do not blacklist)} \\
		      If something fails, be as secure as it does not fail errors / uncertainty should error on the side of the security policy. \textbf{Don't} try to fix on error! (\textit{Automated doors: if they cannot close, stay open})
...
\begin{minipage}[htp]{0.49\textwidth}
	...
	\includegraphics[width=1\textwidth]{assets/permissions.png}\\
...
\begin{minipage}[t]{0.49\textwidth}
	...
	3. Subjects have the permissions of their assigned role\\[0.1em]
...
\hfill
\begin{minipage}[t]{0.49\textwidth}
	\textbf{Permission check order:}\\
	1. If process UID = file owner → check owner bits.\\
	2. Else if GID matches → check group bits.\\
	3. Else → check “other” bits.\\
	\textit{Root (UID 0)} bypasses all checks.\\[0.3em]
	\textbf{Changing Permissions.}\\
	\textbf{chmod.} modify permission bits. \textit{\texttt{chmod +r file}, \texttt{chmod 666 file}}.\\
	...
	\textbf{suid (set user ID).} run program with owner’s privileges.\\
	\texttt{chmod u+s filename}
	\texttt{- rws r-x r-x 1 root root /bin/passwd}\\
	$\rightarrow$ allows normal users to change passwords without full root access.\\
	$\ominus$ dangerous if exploited, in TCB.\\
	\textbf{sgid (set group ID).} run program with group’s privileges.\\
	\texttt{d rwx r-s r-x 2 root staff /srv/shared}\\
	\textbf{sticky bit.} on directories, prevents deleting/renaming files you don’t own (e.g. \texttt{/tmp}).\\
	...
	\textbf{Windows: DACL.} Controls access to objects via list of ACEs (Access Control Entries).
	\begin{itemize}
		\item[]	\textbf{ACE.} \texttt{<Type, Principal, Permissions, Flags>}.
		      \begin{itemize}
		      ...
		      \end{itemize}
		\item[] \textbf{Access Tokens.} Thread/process carries user + group SIDs checked against DACL.
		\item[] \textbf{Least Privilege.} Users run limited by default; elevation via “Run as administrator”
	\end{itemize}
	\textbf{Security Model.} a design pattern for a specific security property or set of properties.
	...
	\textbf{BLP to create Confidentiality Policies.}\\Properties:
	\begin{itemize}
