...
\begin{minipage}[t]{0.495\textwidth}
	...
	\textit{In case of \textbf{Negative Permissions} check negative permissions first before group, think of system crashes before checking negatives.}\\[1px]
...
\begin{minipage}[t]{0.495\textwidth}
	...
	\hline
	\vspace*{1px}
	\textbf{BIBA (Integrity).} Security model where subjects $S$ and objects $O$ are associated to a level of \textbf{Integrity} (trustworthiness). Access rights. Read (Observe), Write (Modify)\\
	...
	$\Rightarrow$ once tainted, subject sinks; prevents writing up thereafter.\\
	...
	\textbf{Access Rules.}\\
	A subject can access an object (read or write) \textbf{only if} the access does not allow\\
	information flow between items with labels in the same conflict set.\\
	...
	\end{minipage}\\[3px]
	\textbf{Sanitization.}\\
	Allows more flexibility by “un-labeling” some items, enabling controlled sharing or reuse of data.
\end{minipage}
\hfill
\begin{minipage}[t]{0.495\textwidth}
	\textbf{Cryptography.}\\
	...
	$C = E_K(M)$ and $M = D_K(C)$\\
	\textbf{Keyspace.} \\The total number of possible keys that can be used in an encryption algorithm.\\
	\textbf{Invertibility Requirment.}\\$\forall K, M, D_K(E_K(M)) = M$ (otherwise message can't be recovered)\\
		...
		So $\log_{2}(25)=4.6$ bits of security, too small for real-world use.\\[3px]
		\textit{The Substitution Cipher}\\
		...
		Both can be broken using \textbf{Frequency Analysis Attack}.\\[1px]
		\textbf{Frequency Analysis.} Use statistical properties of the language.\\
		1. Most frequent letter in English is 'e'. \quad 2. Identify most frequent letter in Ciphertext.\\
		3. Map it to 'e'.\\
		...
		\end{itemize}
		\textbf{Known Plaintext Attack.(KPA)} Active model where the attacker is given access to multiple pairs (M, C) corresponding to messages and their corresponding ciphertext, all encrypted with a secret K.\\
		\textbf{From these pairs, she tries to guess key K to decrypt other messages}
		\textit{realistic because of message headers, }\\[1px]
		\textbf{Chosen Plaintext Attack (CPA)}:\\
		- Suppose Eve convinces Alice to encrypt chosen messages with the secret key\\
		- $\forall m, \text{Eve gets access to } E_{K}(m)$. Eve has access to an Encryption Oracle.\\
		\textit{(realistic - suppose she gets access to an encrypted messaging app for limited time, or to an encryption api). \\very broken if Eve chooses to encrypt entire alphahabet, revealing the key instantly.}\\[1px]
		...
		- Power Analysis - Eve observes the energy consumed by the device doing the crypto.\\[1px]
		\textbf{One-Time-Pad (OTP).}\\
		\textit{Goal - Remove frequency analysis}\\
		- Use a key of random bits as along as the message.\\
		- $Enc(k, m) = m \oplus k$\quad
		- $Dec(k, m) = Enc(k, m) \oplus k$\\
		Key should be random for every sent message.
		Otherwise attacker can collect information about the message.\\[3px]
		OTP is technically \textbf{Perfectly Secure}.\\
		...
		Formally (Perfect Secrecy): $\forall m, c, P(M = m | E_{k}(m)=c) = P(M = m)$.\\
		Guarantees \textbf{confidentiality} \quad $\ominus$ message-length/used once keys.\\[1px]
		...
		\begin{itemize}
			...
			\item[-] \textbf{Reused.} \\The keys is pre-shared once and then reused \textit{(but keys have a "duration")}
			\item[-] \textbf{Must be secret.} \\Reveling the key eliminates any protection provided by the primitive
		\end{itemize}
		\textbf{Stream Ciphers - The pseudo-OTP.} emulate OTP while solving key-length problem.\\
		1.Shared secret short Key (K) \\
		2. Key Stream Generator - Uses K and an Initialization Vector (IV) to produce an arbitrary long, pseudo-random bit stream (S).\\
		(generator needs a key as main seed to generate a predictably random sequence, an iv for the sequence to start differently on each run)\\
	...
	$\ominus$ key stream generators are periodic because seed is finite. Thus, we need period long enough to not an issue (avoiding frequency analysis)\\
		\textbf{Linear Feedback Shift Register for Key Stream Generators}\\
		build a """random""" sequence of bits using a linear recurrence relation on a sequence of bit. For example: Starting with a state $a_{0}, a_{1}, a_{2}, a_{3}$\quad$a_{n} = a_{n-3} \oplus a_{n-4}$\\
	$\oplus$ Easy to build, and to analyze.\\
		\textbf{Randomness of LFSR}\\
	...
	- **distribution property.** As a consequence, the sequence generated exhibit good distribution properties. Every possible non-zero state appears exactly once in a cycle.\\
	However, the underlying operation is **linear**. (some algorithms can recover this with just a subsequence of the stream)
\end{minipage}
\end{document}
