...
\begin{minipage}[t]{0.49\textwidth}
	...
	\begin{enumerate}[leftmargin=10px]
		...
		\item Web Server receives requests and identifies if it's a static asset(.jpg,\dots) or dynamic(.php).
...
\begin{minipage}[t]{0.49\textwidth}
	...
	\end{minipage}\hfill
	\begin{minipage}[t]{0.49\textwidth}
		\textbf{Memory Safety - Spatial Error.} tried access to memory outside the bounds of the allocated region.
		\begin{cc}
			void vulnerable(){
					char buf[12]; char *ptr = buf[11];
					*ptr++=10;
	...
	\begin{minipage}[t]{0.46\textwidth}
		\textbf{CWE 134 - Uncontrolled Format String.} \\untrusted input formated string, lets attacker inject format specifiers that can leak memory contents.
		\begin{cc}
			#include <stdio.h>
	...
	\begin{minipage}[t]{0.52\textwidth}
		"\%4\$p"$\to$Reads 4th parameter even if doesn't exist\\
		"\%6\$n"$\to$Write to address \textbf{pointed to} by 6th parameter
	\end{minipage}
\end{minipage}
\end{document}
