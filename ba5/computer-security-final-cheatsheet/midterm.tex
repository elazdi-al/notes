\documentclass[main.tex]{subtfiles}
\begin{document}
...
\begin{minipage}[t]{0.495\textwidth}
	...
	$\ominus$ role explosion (temptation to create fine grained roles), limited expressiveness, difficult to implement separation of privilege + least privilege.\\
...
\begin{minipage}[t]{0.495\textwidth}
	...
	\textit{on a directory - creating a file inside gets folder's group}\\
	...
	\texttt{sudo} run as root \quad \texttt{su [username]} switch to account(default is root)\\
	\textit{nobody:} UID -2, owns no files, minimal privileges, safer for untrusted code.(least privilege)\\
...
\newpage
\begin{minipage}[t]{0.495\textwidth}
	\textbf{Security Model.} a design pattern for a set of properties. (\warning not covered by model - who are the subjects? what are the objects? what mechanisms to use to implement it?)
	\hline
	\begin{minipage}[t]{0.5\textwidth}
		\textbf{Bell-La Padula (BLP).} \\security model where Subjects S and objects O are associated to a level of \textbf{Confidentiality}. \\
	...
	\begin{minipage}[t]{0.5\textwidth}
		...
		\end{itemize}
	\end{minipage}\\
	\textbf{Declassification.} Removing classification labels from information so it can be shared with lower security levels. \textit{soldier (S) with \textit{Secret} clearance shares sanitized info like “We’ve seen tanks moving” with a lower-level user (U).}
	\hline
	...
	\begin{minipage}[t]{0.5\textwidth}
		...
		\begin{itemize}[itemsep=2pt]
			\item[] \textbf{Simple Integrity Property.}\\
			      If $(s,o,r)$ is a current access, then $\text{level}(o)$ dominates $\text{level}(s)$.\\
			      \textbf{(SUBJECTS CAN'T READ DOWN)} $\Rightarrow$ prevents contamination from low to high.
			\item[]\textbf{$\ast$-Integrity Property.}\\ If $(s,o,w)$ is a current access, then $\text{level}(s)$ dominates $\text{level}(o)$.\\
	...
	\fbox{
		...
		\begin{minipage}[t]{0.54\textwidth}
			\textbf{\textit{BIBA Dominance.}}\\
			(\textit{Reads Most})$L_{\min}=(\textit{Unclassified},\{\})$\\
			(\textit{Writes Most})$L_{\max}=(\textit{Top Secret},\{\textit{NATO},\textit{Crypto},\textit{Nuclear}\})$
		\end{minipage}}\\
	...
	\hline
	\textbf{All together (Asymmetric Cryptography) - Bob sends a message to Alice}\\
	...
	\begin{minipage}[t]{0.49\textwidth}
		...
		Alice uses her secret encryption key $SK_{\text{Alice}}$ to decrypt. $D_{SK_{\text{Alice}}}(E_{PK_{\text{Alice}}}(M)) = M$.\\
	...
	\end{minipage}\\
	\textbf{Authenticity.} only sender can generate a valid signature, allowing the server to verify the sender’s identity and preventing impersonation. \textbf{Integrity.} Any message change invalidates signature, ensuring tampering is detectable. \textbf{Non-repudiation.} only sender can produce the valid signature $\rightarrow$ they can't deny later having sent the message.	\hline
	\begin{minipage}[t]{0.45\textwidth}
		\textbf{Block Cipher - Electronic Code Block (ECB).}\\ encrypt each message block independently using the same key.\\
		1. The message $M$ is divided into blocks.\\$M = M_1 M_2 M_3 M_4$.\\
			2. Each block is encrypted separately $C_i = E_K(M_i)$.\\
			3. Decryption. $M_i = D_K(C_i)$.\\
			\warning If ($M_i = M_j$) $\Rightarrow$ ($C_i = C_j$) $\Rightarrow$ (freq. analysis)
	\end{minipage}\hfill
	\begin{minipage}[t]{0.53\textwidth}
		\textbf{Block Cipher - Cipher Block Chaining (CBC)} \\
		add randomness/inter-block dependence by chaining blocks.\\
		$C_0 = \text{IV}$ \quad	\textbf{Encryption.} $ C_i = E_K(M_i \oplus C_{i-1})$\\
		\textbf{Decryption.}$M_i = D_K(C_i) \oplus C_{i-1}$\\
...
\begin{minipage}[t]{0.495\textwidth}
	...
	\end{minipage}
	\begin{minipage}[t]{0.4\textwidth}
		\textit{Reuse of key Attack.}
		Send two messages of length 5 with key reuse.\\
		$C_1 \oplus C_2 = M_1 \oplus M_2$
	\end{minipage}\\
	\begin{minipage}[t]{0.39\textwidth}
		\textbf{Symmetric Cryptography Schemes}\\Encryption/decryption done with the \textbf{same key}\\
		- \textbf{Block Ciphers.} Operate on fixed-size blocks.\\
	...
	\textbf{Diffie-Hellman Setup.} Alice, Bob (and Eve) know large public parameters:prime p/generator g\\
	...
	\begin{minipage}[t]{0.35\textwidth}
		\textbf{Block Cipher - Counter Mode}\\add randomness per-block without inter-block dependencies. \\\textbf{(parallel)}\\
		\textbf{Encrypt.} $C_i = M_i \oplus E_K(\text{Nonce}+i)$.\\
		\textbf{Decrypt.} $M_i = C_i \oplus E_K(\text{Nonce}+i)$.\\
		Nonce must be unique (never with same key) reuse leaks keystream.\hline
		\textbf{Message Authentication Code (MAC).} short tag computed from a message and secret key(pre-shared) to verify message \textbf{integrity} and \textbf{authenticity}.\\
	\end{minipage}
	\begin{minipage}[t]{0.65\textwidth}
		\textbf{Block Cipher - CBC-MAC.} turns block cipher into MAC by chaining message blocks with encryption.\\
		$C_0 = IV $\;\textbf{Encrypt.} $C_i = E_K(M_i \oplus C_{i-1})$\;$\text{MAC}_K(M_1, \ldots, M_n) = C_n$\\
		deterministic. fixed IV $\rightarrow$ same message/same MAC.\\
		\warning message length must be known and fixed; otherwise, extension attacks are possible (e.g., using $M | (T \oplus M')$).
		\hline
		\textbf{1. Encrypt-and-MAC.} send $E_K(P_1P_2)$ and $MAC_K(P_1P_2)$. integrity of plaintext but no integrity on ciphertext/may reveal info on ciphertext. \textbf{2. MAC-then-Encrypt} send $E_K(P_1P_2||MAC_K(P_1P_2))$. hides MAC and provides confidentiality/integrity on plaintext but no integrity ciphertext.\\
		\textbf{3. Encrypt-then-MAC} send $E_K(P_1P_2)||MAC_K(E_K(P_1P_2))$. \textbf{authenticates} ciphertext, ensures \textbf{confidentiality/integrity} \checkmark
	\end{minipage}
\end{minipage}
\end{document}
